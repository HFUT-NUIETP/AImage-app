<template>
	<view class="draw">
		<canvas canvas-id="myDraw" disable-scroll="true" style="width: 750rpx; height: 750rpx" @touchstart="onStart"
			@touchmove="onMove" @touchend="onStop"></canvas>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
				...
				if (this.useMethod === "paint") {
					...
					const lineFill = (x, y, newColor, oldColor) => {
						...
						const isSameColor = (index) => {
							return pixels.data[index + 0] === oldColor[0] && pixels.data[index + 1] ===
								oldColor[1] && pixels.data[index + 2] === oldColor[2] && pixels.data[index +
								3] === oldColor[3]
						}
						let dirList = [
							[x, y]
			...
			onStop() {
			  if (this.useMethod === 'paint_bucket') return;
				this.s = [];
				console.log("Backup Image");
				uni.canvasGetImageData({
					x: 0,
